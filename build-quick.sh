...
# build-quick.sh: Quick development build script for MyCppLibraryProject
#
# Purpose:
    ...
    if [ -x "$(command -v lcov)" ] && [ "${ENABLE_COVERAGE}" = "ON" ]; then
        ...
        if [ -x "$(command -v genhtml)" ]; then
            ...
            genhtml coverage.info                            \
                    ...
                    --branch-coverage                        \
                    --legend                                 \
                    --title  "MyCppLibraryProject Coverage Report" \
                    --prefix $(pwd)/..                       \
                    --rc     branch_coverage=1
...
echo -e "\nOutput files:"
echo -e "  - Library: ${BLUE}$(pwd)/../dist/libmycpplibrary_lib.a${NC}"
if [ "${BUILD_TESTS}" = "ON" ]; then
    echo -e "  - Test executable: ${BLUE}$(pwd)/mycpplibrary_tests${NC}"
fi
